...
SZ_RESULT SafeReadDirect(ISzInStream *inStream, Byte *data, size_t size)
{
  ...
  while (size > 0)
  {
    ...
    RINOK(inStream->zRead(inStream, (void **)&inBuffer, size, &processedSize));
  ...
  #else
  size_t processedSize;
  RINOK(inStream->zRead(inStream, data, size, &processedSize));
...
SZ_RESULT SzReadTime(CSzData *sd, UInt32 numFiles, CFileItem *files, 
	Byte ** TimeVector, UInt64 type, void * (*allocFunc)(size_t size))
{
  UInt32 i;
  RINOK(SzReadBoolVector2(sd, numFiles, TimeVector, allocFunc));
    ...
    ISzAlloc *allocTemp)
{
  ...
  while(1)
  {
    ...
    switch((int)type)
    {
      ...
      case k7zIdLastAccessTime:
      {
        RINOK(SzReadTime(sd, numFiles, files, TimeVector, type, allocTemp->Alloc));
        break;
      }
#endif
      default:
    ...
    ISzAlloc *allocTemp)
{
  ...
  SZ_RESULT res = SzReadHeader2(sd, db, 
      ...
      &emptyStreamVector, &emptyFileVector, &TimeVector,
      allocMain, allocTemp);
    ...
    ISzAlloc *allocTemp)
{
  ...
  RINOK(inStream->zSeek(inStream, dataStartPos));
    ...
    ISzAlloc *allocTemp)
{
  ...
  */
  ...
  RINOK(inStream->zSeek(inStream, (CFileSize)(pos + nextHeaderOffset)));
